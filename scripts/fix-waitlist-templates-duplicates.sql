-- Fix Waitlist Email Templates (Handles Duplicates)
-- Run this in Supabase SQL Editor

-- Step 1: Delete old Launch Notification templates (keep newest)
DELETE FROM email_templates 
WHERE name = 'Launch Notification' 
AND id NOT IN (
...
-- Step 2: Delete old Coming Soon Waitlist templates (keep newest)
DELETE FROM email_templates 
WHERE name = 'Coming Soon Waitlist' 
AND id NOT IN (
  SELECT id FROM email_templates 
  WHERE name = 'Coming Soon Waitlist' 
  ORDER BY created_at DESC 
...
-- Step 3: Update the remaining Launch Notification template
UPDATE email_templates 
SET 
  subject = 'InPlayTV is now LIVE! 🚀',
...
-- Step 4: Update the remaining Coming Soon Waitlist template
UPDATE email_templates 
SET 
  subject = 'Thanks for joining the InPlayTV waitlist! 🎉',
  content = '<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
-- Step 5: Add unique constraint to prevent future duplicates (skip if exists)
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Step 6: Verify - should only see 1 of each now
