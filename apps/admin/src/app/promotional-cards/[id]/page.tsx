...
export default function EditPromotionalCardPage() {
  ...
  const fetchCard = async () => {
    try {
      const res = await fetch(`/api/promotional-cards/${cardId}`);
      if (!res.ok) throw new Error('Failed to fetch card');
      
      const card: PromotionalCard = await res.json();
      ...
      setFormData({
        ...
        prize_pool_display: card.prize_pool_display || '',
        entries_display: card.entries_display || '',
        entry_fee_display: card.entry_fee_display || '',
        first_place_display: card.first_place_display || '',
        ...
        badge_style: card.badge_style || 'gold',
      });
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        <div style={{ color: 'rgba(255,255,255,0.6)' }}>Loading card...</div>
      </div>
    );
  }
    ...
    <div style={{ padding: '2rem', maxWidth: '800px' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.875rem' }}>
          Update the promotional tournament card details
        </p>
      </div>
